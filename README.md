...
### 健康场景（device_id_1 ~ device_id_9）

| 设备 | 场景名称 | 说明 |
|------|---------|------|
| device_id_1 | 完全正常 | 180 天抓挠均值约 10 次/天，基线稳定，无报警 |
| device_id_2 | 急性皮肤病后康复 | 第 60~80 天发病（均值 30 次/天），之后恢复正常；测试报警触发与康复 |
| device_id_3 | 慢性皮肤病（不恢复） | 第 60 天起持续高抓挠（28 次/天），全程不恢复；测试持续异常检测 |
| device_id_4 | 复发（两次发病） | 第 40~55 天、第 120~135 天各发病一次；测试多次发病检测 |
| device_id_5 | 渐进性过敏（基线追上） | 三阶段渐进升高（10→15→22 次/天），基线逐步跟上；测试慢性过敏识别 |
| device_id_6 | 食物过敏（突发触发） | 第 90 天突然发病，之后持续高位（25 次/天）；测试突发型报警 |
| device_id_7 | 跳蚤/螨虫侵扰 | 第 50~80 天极高抓挠（45 次/天），康复后恢复；测试极端异常检测 |
| device_id_8 | 季节性过敏（高温度系数） | 温度系数 0.35，高温时抓挠自然升高；测试温度修正防误报 |
| device_id_9 | 术后恢复 | 第 30~90 天活动受限（3 次/天），其余正常；测试低活动期基线学习 |

### 设备/数据质量场景（device_id_10 ~ device_id_16）

| 设备 | 场景名称 | 说明 |
|------|---------|------|
| device_id_10 | 忘记佩戴（短缺口） | 第 35~37 天未佩戴（3 天缺口）；测试短缺口后基线恢复 |
| device_id_11 | 电池耗尽 | 第 40~44 天没电（5 天缺口）；测试中等缺口处理 |
| device_id_12 | 长期缺口（>30 天） | 第 30~64 天缺口（35 天）；触发有效天重置，测试基线重建 |
| device_id_13 | 信号不稳定 | 第 20~80 天间歇性信号丢失；缺口随机分布，测试断续数据处理 |
| device_id_14 | 松动项圈 | 第 50~57 天数据无效（8 天）；data_quality=4，测试无效数据标记 |
| device_id_15 | 设备更换 | 第 88~91 天缺口，模拟更换设备；缺口后基线状态重置 |
| device_id_16 | 传感器漂移 | 第 70~90 天传感器异常偏高（35 次/天），之后恢复；data_quality=3 |

### 环境场景（device_id_17 ~ device_id_20）

| 设备 | 场景名称 | 说明 |
|------|---------|------|
| device_id_17 | 季节转换（明显温度效应） | 温度系数 0.30，整体随季节温度波动；测试高温度系数处理 |
| device_id_18 | 搬家（环境突变） | 第 60 天起环境温度整体 +5°C；抓挠略升（13 次/天），基线重新适应 |
| device_id_19 | 出行旅游（短缺口+不同环境） | 第 80~89 天缺口（外出旅行）；测试旅行期间数据中断 |
| device_id_20 | 高湿度环境 | 整体抓挠基线偏高（14 次/天）；测试个体差异化基线建立 |

### 个体类型场景（device_id_21 ~ device_id_24）

| 设备 | 场景名称 | 说明 |
|------|---------|------|
| device_id_21 | 幼犬（基线建立慢） | 热身期延长至 7 天；高活跃度（15 次/天）；测试幼犬个体化配置 |
| device_id_22 | 老年犬（低活动） | 极低抓挠基线（5 次/天），温度系数 0.05；测试低活动个体的灵敏度 |
| device_id_23 | 高活跃度犬 | 高抓挠基线（20 次/天），温度系数 0.12；测试高活跃个体的异常判断 |
| device_id_24 | 低活跃度犬（敏感） | 极低基线（4 次/天），温度系数 0.08；小波动即可触发 z-score 异常 |
...
- **热身期**：默认前 3 天（device_id_21 为 7 天），只收集数据，不输出评估结果
